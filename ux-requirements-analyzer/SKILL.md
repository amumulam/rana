...
| 维度 | 核心问题 | 通过标准 |
|------|---------|---------|
| **正确性** | 需求是否真实反映业务和用户问题？ | 无明显理解偏差 |
| **完整性** | 主流程/异常/边界/依赖是否全覆盖？ | 33项检查清单通过率 ≥ 80% |
| **可执行性** | 能否直接支撑后续设计分析？ | 无模糊表述（「适当」「合理」等词） |
...
- 全部 **PASS** → 进入 Stage 5
- 有 **FAIL**（核心 P0 缺口未补完）→ 返回 Stage 3 补充，标注缺失项后重新验证
- 有 **FAIL**（完整性 < 80% 但所有 P0 缺口已补完）→ 可酌情推进 Stage 5，但需在 quality-report.md 和 final-analysis.md 中明确标注剩余 FAIL 项为「已知风险，待后续迭代补充」
- 有 **WARN** → 可继续，但需在最终文档标注风险点
...
### 完整性：✗ FAIL
- 检查清单通过率：24/33（73%，未达 80%）
- 缺失项：
  - [ ] 加载状态未定义
  - [ ] 网络异常处理未定义
- **修正建议**：返回 Stage 3 补充上述两项
...
完整性维度 FAIL，需返回 Stage 3 补充后重新验证。
```
