...
- 若用户尚未提供任何信息，直接进入 R3b 询问产品线

**Step R3a（匹配成功）：按需读入知识内容**

1. 访问该产品线页面，列出所有文档标题
2. 优先读入 `overview` 概览文档（如存在）
3. 根据需求关键词，再选 1-2 个最相关的专项文档读入
4. 知识库内容作为分析上下文，**不直接向用户输出，不在分析文档中重复抄写**
5. 在后续生成的 `input-structured.md` 头部注明参考文档（见模板）
...
> - 这个需求属于哪个产品线？（如：`vivo-official-website`）
> - 是否有特定业务模块需要参考？
>
> 如果知识库暂无相关内容，回复「跳过」继续分析。

- 用户指定产品线后：重新执行 R2 → R3a
- 产品线存在但无匹配文档：告知「该产品线暂无文档，建议后续补充」，继续分析
- 用户回复「跳过」：直接进入「接收输入」

**Step R4：用户可纠正**

若用户指出参考文档有误，重新读入指定文档，更新 `input-structured.md` 中的参考知识库注明。
...
**版本/迭代号**：[版本号]
**分析日期**：YYYY-MM-DD
**参考知识库**：http://10.109.65.184:3000/zh-context/<产品线>/<文档名>（知识库为空或跳过时省略此行）
