...
**触发条件**：存在 P0 缺口、逻辑疑点、Must-Challenge 条件被触发，**或存在推断项/存疑点需确认**

**重要原则**：即使无 P0 缺口，每个阶段结束后仍需与用户确认推断项和存疑点，不可直接跳至下一阶段。

**对话流程**：

```
发现缺口/疑点/推断项 → 呈现并提问 → 用户回应 →
判断是否已补齐/确认 →
  ├─ 已补齐/确认：输出确认，进入下一阶段
  └─ 未补齐：继续下一轮提问（循环）
```
...
- HMW 思维提供替代方案（方案层尤其重要）
- 遇用户反驳，执行批判反驳 5 步流程（见本文件 §标准化 5 步批判反驳流程）
- 记录变更到 change-log.md

各阶段的批判重点和协作深挖方向见本文件 §分阶段批判重点 和 §分阶段协作深挖方向。

---

## 四段式独立消息通用结构

每个阶段结束时，按以下四段式结构输出独立消息：

**消息块 1（引导语）**：各阶段自定义引导语（见各 stage guideline）

**消息块 2（报告本体）**：
- 先将分析内容写入目标文件（final-analysis.md 或 quick-analysis.md），再从文件中静默读取
- 将读取到的全文写入当前对话的一条 message block（即 AI 的文本回复），不省略、不截断
- ⚠️ **唯一允许的呈现方式**：由 AI 将文件内容写入 message block。禁止使用 tool call output、代码块包裹、文件链接等其他方式呈现。原因：不同 IM / 消息平台对 tool output 的渲染支持不一致，message block 是唯一跨平台可靠的呈现方式。
- 不输出其他阶段章节内容
...
- 无 P0 缺口：`✓ 无 P0 关键信息缺失`
- 有 P0 缺口：呈现缺口表格
- 无论有无缺口，均需列出带有 `[分析推断]` 标签的待确认项
- 有缺口/待确认 → 进入协作讨论环节
- 无缺口但有待确认项 → 进入协作确认环节（确认推断项）
- 无缺口且无待确认项 → 确认后进入下一阶段
...
**消息块 4（确认提示语）**：各阶段自定义确认语（见各 stage guideline）

**Full Mode 差异**（默认）：
- 消息块 1-4 的具体内容见各 stage guideline（`references/stage-1-diagnosis.md`、`references/stage-2-solution.md`、`references/stage-3-refine.md`）

**Quick Mode Step 1 差异**：
- 消息块 1：`正在为你展示快速分析（四维度：问题→目标→供给→指标）`
- 消息块 2：读取 quick-analysis.md 中四维度分析表格 + 总结，逐字全文写入 message block
- 消息块 3：列出 ⚠️ 标注项（数据缺失 / 逻辑未明确），转化为待确认项
- 消息块 4：`📄 以上是四维度快速分析。如需调整任何内容，请直接告知。`

**Quick Mode Step 2→3 差异**：
- 消息块 1：`正在为你展示讨论后的更新分析`
- 消息块 2：读取 quick-analysis.md 中修正后的四维度表格 + 总结 + [经讨论确认] 标注要点，逐字全文写入 message block
- 消息块 3：列出剩余待确认项（如仍有 ⚠️ 标注未消解）
- 消息块 4：`📄 以上是讨论后的更新分析。若确认无误，将进入 Step 3 输出评审提问清单。`

---

## 文件内容呈现约束

无论在任何模式（Quick/Full）、任何阶段（Stage 1/2/3）、任何环节，**当用户主动声明要求查看某个文件的完整内容时**，AI 必须遵守：

1. 内部静默读取目标文件
2. 将全文写入一条独立的 message block（即 AI 的文本回复）返回给用户

⚠️ **唯一允许的呈现方式**：由 AI 将文件内容写入 message block。禁止以下方式：
- 工具调用输出（tool call output）
- 代码块包裹
- 文件 URL / 链接地址
- 任何依赖特定平台渲染能力的呈现方式

**原因**：不同 IM / 消息平台对 tool output、文件链接的渲染支持不一致。message block 是唯一跨平台可靠的呈现方式。
